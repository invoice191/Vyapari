...
const drawPDFHeader = (doc: any, payload: ReportPayload, pageNumber: number, totalPages: number) => {
  ...
  doc.text(payload.businessName, 38, 15);
  doc.setFontSize(8);
  ...
  doc.setFont('helvetica', 'bold');
  doc.text(payload.title, w - 15, 15, { align: 'right' });
  doc.setFontSize(8);
  doc.setTextColor(100, 116, 139);
  doc.setFont('helvetica', 'normal');
  ...
  if (payload.isDraft) {
    ...
    doc.setFont('helvetica', 'bold');
    doc.text('DRAFT', w / 2, 180, { align: 'center', angle: 45 });
  }
};
...
const drawKPIRow = (doc: any, kpis: ReportPayload['kpis'], startY: number): number => {
  ...
  kpis.slice(0, 4).forEach((kpi, i) => {
    ...
    doc.setTextColor(100, 116, 139);
    doc.setFont('helvetica', 'normal');
    doc.text(kpi.label.toUpperCase(), x + boxW / 2, startY + 6, { align: 'center' });
    doc.setFontSize(11);
    doc.setTextColor(...SLATE_900);
    doc.setFont('helvetica', 'bold');
    doc.text(String(kpi.value), x + boxW / 2, startY + 14, { align: 'center' });
  });
  return startY + 26;
};
...
export async function downloadPDF(
  ...
  doc.autoTable({
    ...
    foot,
    startY: margins.top + (payload.kpis?.length ? 28 : 2) + (chartImage ? 70 : 0) + (template === 'simple' ? 0 : 4),
    margin: { top: margins.top, bottom: margins.bottom, left: margins.left, right: margins.right },
    styles: { font: 'helvetica', fontSize: 8.5, cellPadding: 3.5, overflow: 'linebreak' },
    headStyles: { fillColor: INDIGO, textColor: [255, 255, 255], fontStyle: 'bold', fontSize: 8.5, lineWidth: 0 },
    alternateRowStyles: { fillColor: SLATE_50 },
    footStyles: { fillColor: INDIGO_LIGHT, textColor: SLATE_900, fontStyle: 'bold' },
    ...
    didDrawPage: (data: any) => {
      ...
      if (data.pageNumber === 1 && payload.kpis?.length && template !== 'simple') {
        drawKPIRow(doc, payload.kpis, margins.top + 2);
      }
