...
import { 
  ...
  MessageSquare, ArrowRight, RefreshCw, Loader2, Check
} from 'lucide-react';
...
export const WarRoomEngine: React.FC = () => {
  ...
  const handleExecuteAction = async (alert: TacticalAlert) => {
    setExecutingId(alert.id);
    toast.info(`Deploying defensive parameters for '${alert.title}'...`, 'Strategic Command');
    
    // Simulate real strategic resolution execution
    await new Promise(r => setTimeout(r, 1500));
    
    setDeployedAlerts(prev => ({ ...prev, [alert.id]: true }));
    setExecutingId(null);
    toast.success(`Counter-action deployed successfully: ${alert.impact}`, 'Threat Quarantined');
  };
            ...
            <div className="lg:col-span-8 space-y-6">
               ...
               {alerts.length === 0 ? (
                 ...
                 <div className="space-y-4">
                    {alerts.map((alert, i) => {
                       const isDeployed = deployedAlerts[alert.id];
                       return (
                        <motion.div 
                          key={alert.id}
                              ...
                              <div className="flex-1">
                                 ...
                                 <div className="flex items-center justify-between pt-4 border-t border-white/5">
                                    ...
                                    <button 
                                      ...
                                      disabled={isDeployed || executingId === alert.id}
                                      className={`px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all shadow-lg flex items-center gap-2 ${
                                        isDeployed 
                                    ...
                                    >
                                       {executingId === alert.id ? (
                                         ...
                                         <>
                                           <span>Execute Counter-Action</span>
                                           <Crosshair size={12} />
                                         </>
                                       )}
                                    </button>
               ...
               <div className="bg-slate-900/40 border border-white/5 rounded-[2.5rem] p-8 relative overflow-hidden group">
                  ...
                  <div className="flex items-end gap-3 mb-2">
                     <span className={`text-5xl font-black italic tracking-tighter ${metrics?.velocity < 1 ? 'text-rose-500' : 'text-emerald-500'}`}>
                        {((metrics?.velocity || 0) * 100).toFixed(1)}%
                     </span>
