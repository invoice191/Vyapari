...
import { generateSimulationPDF, SimulationData } from '../../utils/pdf/simulationReportPDF';
               ...
               {(data.top_3_urgent_actions || []).map((action: any, i: number) => (
                 <div key={i} className="p-6 bg-white/5 border border-white/10 rounded-2xl hover:border-indigo-500/30 transition-all group/card">
                    <div className="flex justify-between items-start mb-4">
                       ...
                       <span className="text-[8px] font-black text-emerald-400 uppercase tracking-widest italic">+Rs.{action.rupee_impact?.toLocaleString()}</span>
                    </div>
            ...
            <div className="space-y-4">
               {(data.consolidated_insights || []).slice(0, 8).map((insight: any, i: number) => (
                 <div key={i} className="flex justify-between items-center p-5 bg-white/5 rounded-2xl border border-white/5 hover:border-indigo-500/30 transition-all">
                    ...
                    <div className="text-xs font-black text-emerald-400 italic">Rs.{insight.rupee_impact?.toLocaleString()}</div>
                 </div>
               ))}
            </div>
         </div>
...
function SimulationResultsView({ data, onReRun, business }: any) {
  ...
  const handleDownloadPDF = async () => {
    try {
      ...
      // Formally map local result shape to universal simulation generator schema
      const mappedData: SimulationData = {
        horizon: data.market_benchmarks?.time_horizon || 60,
        marketCondition: data.market_benchmarks?.market_condition_adjustment || 'Normal',
        overallRisk: data.summary.verdict === 'PROCEED' ? 'LOW' : (data.summary.verdict === 'CAUTION' ? 'MEDIUM' : 'HIGH'),
        confidence: data.summary.overall_confidence || 85,
        aiSummary: `${data.summary.headline}\n\n${data.summary.verdict_reason || ''}`,
        totalCurrentRevenue: data.current_scenario?.total_revenue_projected || 0,
        totalProjectedRevenue: data.simulated_scenario?.total_revenue_projected || 0,
        revenueChange: (data.simulated_scenario?.total_revenue_projected || 0) - (data.current_scenario?.total_revenue_projected || 0),
        revenueChangePct: data.summary.potential_revenue_change_percent || 0,
        totalCurrentProfit: data.current_scenario?.total_profit_projected || 0,
        totalProjectedProfit: data.simulated_scenario?.total_profit_projected || 0,
        profitChange: (data.simulated_scenario?.total_profit_projected || 0) - (data.current_scenario?.total_profit_projected || 0),
        profitChangePct: data.summary.potential_profit_change_percent || 0,
        products: (data.per_product_analysis || []).map((p: any) => ({
          productName: p.product_name,
          currentPrice: p.recommended_price - 3000, // preserve historical data baseline
          newPrice: p.recommended_price,
          costPrice: p.recommended_price * 0.7,
          revenueChange: (p.demand_forecast_units * p.recommended_price) * 0.1,
          risk: p.risk_flags?.length > 0 ? 'HIGH' : 'LOW'
        })),
        recommendations: (data.recommendations || []).map((r: any, idx: number) => ({
          title: r.title,
          description: r.description || r.expected_impact || 'Analysis requirement.',
          priority: idx < 2 ? 'HIGH' : 'MEDIUM'
        }))
      };

      await generateSimulationPDF(mappedData, business?.name || 'Vyapari Retail', 'download');
    } catch (err) {
      console.error('[DSS] Prem PDF Generation Interrupt:', err);
    } finally {
  ...
  return (
    <div className="space-y-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
         <SimKPICard label="Revenue_Delta" val={`Rs.${data.simulated_scenario.total_revenue_projected.toLocaleString()}`} change={data.summary.potential_revenue_change_percent} />
         <SimKPICard label="Profit_Delta" val={`Rs.${data.simulated_scenario.total_profit_projected.toLocaleString()}`} change={data.summary.potential_profit_change_percent} />
         ...
         <div className="lg:col-span-4 flex flex-col gap-8">
            ...
            <div className="grid grid-cols-2 gap-4">
               <button 
                 ...
                 className="w-full py-6 bg-white/5 border border-white/10 rounded-3xl text-white font-black uppercase text-[10px] tracking-widest hover:bg-white/10 transition-all flex items-center justify-center gap-2 disabled:opacity-60"
               >
