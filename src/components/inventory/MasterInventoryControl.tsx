...
export default function MasterInventoryControl() {
  ...
  const handleDownloadReport = (product: any) => {
    ...
    doc.save(`${product.name.replace(/\s+/g, '_')}_Intelligence_Audit.pdf`);
  ...
  const handleUpdateProductPrice = async (product: any, newPrice: number) => {
    ...
    try {
      ...
      toast(`Price updated to ₹${newPrice} for ${product.name}`, "success");
      refresh('products');
    } catch (err: any) {
  ...
  const getPromoMessage = (customerName: string) => {
    ...
    switch (campaignType) {
      case 'flash':
        return `⚡ FLASH SALE: Hi ${customerName}! Get ${promoDiscount}% OFF on ${promoProduct.name}. Now only ₹${discountedPrice}. Valid for 24h at ${bizName}. Don't miss out!`;
      case 'loyalty':
      ...
      case 'clearance':
        return `🔥 CLEARANCE: We're clearing stock! Grab ${promoProduct.name} at a massive discount. Now only ₹${discountedPrice}. Visit ${bizName} today!`;
      case 'restock':
  ...
  const getGlobalMessage = (customerName: string) => {
    ...
    if (featuredProduct) {
      ...
      return `📢 SPECIAL ANNOUNCEMENT: Hi ${customerName}! We have a special offer on ${featuredProduct.name} at ${bizName}. Get it for just ₹${discountedPrice}. Visit us today!`;
    }
        ...
        <div className="relative z-10 flex flex-col md:flex-row justify-between items-center gap-10">
          ...
          <div className="flex gap-4">
            <div className="p-6 bg-white/5 border border-white/10 rounded-3xl text-center min-w-[140px] backdrop-blur-xl">
               ...
               <div className="text-2xl font-black text-white">₹{(products.reduce((acc, p) => acc + (p.selling_price * getStockQty(p)), 0) / 1000).toFixed(1)}K</div>
            </div>
        ...
        <div className="space-y-6 mb-10">
          ...
          <div className="flex flex-col lg:flex-row justify-between items-stretch lg:items-center gap-4">
            <div className="flex flex-wrap gap-2">
               ...
               <button className="px-5 py-2.5 bg-white text-slate-500 border border-slate-200 rounded-xl font-black text-[10px] uppercase tracking-widest hover:border-slate-300 transition-all">
                 Export Stock List
               </button>
            </div>
        ...
        {promoProduct && (
          ...
          >
            <motion.div
              ...
              animate={{ scale: 1, y: 0 }}
              exit={{ scale: 0.9, y: 20 }}
              className="bg-white/95 border border-slate-200/50 rounded-[2.5rem] p-8 max-w-4xl w-full shadow-2xl shadow-slate-900/10 space-y-6 relative"
                ...
                <div className="space-y-6">
                  ...
                  <div className="p-4 bg-slate-50 border border-slate-100 rounded-3xl flex items-center justify-between gap-4">
                    <div>
                      ...
                      <div className="text-[10px] text-slate-400 font-bold mt-1">Price: ₹{promoProduct.selling_price || 0}</div>
                    </div>
                    <div className="flex items-center gap-3">
                      ...
                      <div className="text-right">
                        <div className="text-[10px] font-black text-indigo-600">₹{Math.round((promoProduct.selling_price || 0) * (1 - promoDiscount / 100))}</div>
      ...
      <AnimatePresence>
        {showAddProductModal && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 bg-slate-950/80 backdrop-blur-md z-[1100] flex items-center justify-center p-4 overflow-y-auto"
          ...
          >
            <motion.div
              ...
              onClick={e => e.stopPropagation()}
              className="relative bg-white border border-slate-200/80 shadow-[0_24px_70px_rgba(15,23,42,0.15)] rounded-3xl overflow-hidden max-w-4xl w-full max-h-[90vh] flex flex-col text-slate-800"
            >
              {/* Header */}
                      ...
                      <div className="h-full bg-slate-950 text-white p-5 rounded-2xl flex flex-col justify-between border border-slate-800 shadow-inner relative overflow-hidden font-mono min-h-[320px]">
                        ...
                        <div>
                          ...
                          <div className="flex justify-between items-center border-b border-slate-800 pb-2 mb-4">
                            ...
                            <span className={`text-[8px] font-black uppercase tracking-wider px-2 py-0.5 rounded ${
                              ...
                              {marginLabel === 'healthy' ? '⚡ High Yield' : marginLabel === 'low' ? '⚠️ Low Margin' : '🛑 Unprofitable'}
                            </span>
                          </div>
                          ...
                          <div className="mt-4 pt-3 border-t border-slate-900 space-y-2">
                            ...
                            <div className="flex justify-between items-center text-[10px]">
                              ...
                              <span className="text-slate-300">₹{totalCost.toLocaleString('en-IN')}</span>
                            </div>
                            
                            <div className="flex justify-between items-center text-[10px]">
                              <span className="text-slate-500">Gross Asset Yield:</span>
                              <span className="text-slate-300">₹{totalYield.toLocaleString('en-IN')}</span>
                            </div>
                        ...
                        <div className="mt-5 p-2.5 bg-slate-900/60 border border-slate-800 rounded-xl">
                          <p className="text-[9px] text-slate-400 font-medium leading-relaxed">
                            {profit > 0 ? (
                              `✓ This asset yields ₹${profit} per unit. With ${newProduct.quantity} units, you stand to make a premium ₹${projectedProfit} profit on initial stock. Excellent pricing metrics.`
                            ) : profit < 0 ? (
                              ...
                              "ℹ Enter cost and selling price to compute dynamic margin valuation metrics automatically."
                            )}
                          </p>
                        </div>
                      </div>
        ...
        {showAdjustModal && adjustProduct && (
          ...
          >
            <motion.div
              ...
              exit={{ scale: 0.95, y: 20 }}
              onClick={e => e.stopPropagation()}
              className="bg-white border border-slate-200/80 shadow-2xl rounded-[2.5rem] p-8 max-w-md w-full space-y-6 relative overflow-hidden"
            >
      ...
      <AnimatePresence>
        {showGlobalBroadcast && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 bg-slate-950/80 backdrop-blur-md z-[1200] flex items-center justify-center p-4 overflow-y-auto"
          ...
          >
            <motion.div
              ...
              animate={{ scale: 1, y: 0 }}
              exit={{ scale: 0.95, y: 20 }}
              className="bg-white/95 border border-slate-200/50 rounded-[3rem] p-10 max-w-5xl w-full shadow-2xl space-y-8 relative"
              onClick={e => e.stopPropagation()}
            >
               <button 
                onClick={() => setShowGlobalBroadcast(false)}
                       ...
                       {products.slice(0, 10).map(p => (
                         ...
                         >
                            <div>
                              ...
                              <div className="text-[8px] text-slate-400">₹{p.selling_price}</div>
                            </div>
      ...
      <AnimatePresence>
        {showInsightsModal && insightProduct && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 bg-slate-950/80 backdrop-blur-xl z-[1300] flex items-center justify-center p-4"
          ...
          >
            <motion.div
              ...
              onClick={e => e.stopPropagation()}
              className="bg-white shadow-[0_50px_100px_-20px_rgba(0,0,0,0.3)] rounded-[3rem] w-full max-w-6xl max-h-[95vh] overflow-hidden flex flex-col relative"
            >
              <ProductInsights 
