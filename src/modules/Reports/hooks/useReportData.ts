      ...
      switch (reportId) {
        case 'daily-sales': {
          ...
          fetchedData = dailyList.map(r => ({
            ...
            discounts: `₹${r.disc.toLocaleString()}`,
            net: `₹${r.net.toLocaleString()}`,
            tax: `₹${r.tax.toLocaleString()}`,
            profit: `₹${r.profit.toLocaleString()}`,
            avg: `₹${(r.net / r.txns).toLocaleString()}`
          }));
          ...
          generatedCards = [
            { label: 'Total_Net_Revenue', value: `₹${totalNet.toLocaleString()}`, trend: '+14.2%', color: 'orange' },
            { label: 'Avg_Ticket_Value', value: `₹${totalTxns ? Math.round(totalNet / totalTxns) : 0}`, trend: '+2.1%', color: 'blue' },
            { label: 'Gross_Discount_Burn', value: `₹${dailyList.reduce((s, r) => s + r.disc, 0).toLocaleString()}`, trend: '+15.4%', negative: true, color: 'red' },
            { label: 'Overall_Net_Profit', value: `₹${totalProfit.toLocaleString()}`, trend: 'OPTIMAL', color: 'green' }
          ];
          recs = [
          ...
          sumLines = [
            `Overall net sales are ₹${totalNet.toLocaleString()}, processed across ${totalTxns} transactions.`,
        ...
        case 'hourly-sales':
          fetchedData = R.hourly.map(r => ({
            ...
            avgTicket: `₹${r.avgTicket.toLocaleString()}`,
          ...
          generatedCards = [
            ...
            { label: 'Avg_Hourly_Sales', value: '₹28,500', trend: 'STABLE', color: 'green' },
        ...
        case 'payment-modes': {
          ...
          fetchedData = modeList.map(r => ({
            ...
            contribution: `${r.pct}%`,
            avgTicket: `₹${r.txns ? Math.round(r.amount / r.txns).toLocaleString() : 0}`
          }));

          generatedCards = [
            { label: 'Dominant_Channel', value: modeList[0]?.method || 'N/A', trend: `${modeList[0]?.pct || 0}% SHARE`, color: 'orange' },
            { label: 'Total_Digital_Volume', value: `₹${modeList.filter(m => m.method.toLowerCase() !== 'cash').reduce((s, m) => s + m.amount, 0).toLocaleString()}`, trend: 'REAL-TIME', color: 'green' },
            { label: 'Cash_on_Hand_Est', value: `₹${modeMap['Cash']?.amount.toLocaleString() || 0}`, trend: 'STABLE', color: 'blue' },
            { label: 'Total_Collections', value: `₹${totalAmt.toLocaleString()}`, trend: 'AUDITED', color: 'green' }
          ];
          recs = [
          ...
          sumLines = [
            `Total collections across all modes reached ₹${totalAmt.toLocaleString()}.`,
        ...
        case 'tax-gst': {
          ...
          fetchedData = [
            { category: 'Domestic Sales', taxableAmt: `₹${taxableTotal.toLocaleString()}`, cgst: `₹${cgstTotal.toLocaleString()}`, sgst: `₹${sgstTotal.toLocaleString()}`, igst: `₹${igstTotal.toLocaleString()}`, cess: '₹0', total: `₹${totalTax.toLocaleString()}`, rate: 'Mixed' }
          ];

          generatedCards = [
            { label: 'Total_Tax_Liability', value: `₹${totalTax.toLocaleString()}`, trend: 'LIVE FEED', color: 'orange' },
            { label: 'CGST_Liability', value: `₹${cgstTotal.toLocaleString()}`, trend: 'AUDITED', color: 'blue' },
            { label: 'SGST_Liability', value: `₹${sgstTotal.toLocaleString()}`, trend: 'AUDITED', color: 'green' },
            { label: 'Total_Taxable_Value', value: `₹${taxableTotal.toLocaleString()}`, trend: 'STABLE', color: 'blue' }
          ];
          recs = [
          ...
          sumLines = [
            `Total taxable value of ₹${taxableTotal.toLocaleString()} processed in the current period.`,
            `Cumulative tax obligation stands at ₹${totalTax.toLocaleString()}.`
          ];
          break;
        }
        ...
        case 'promo-effectiveness':
          fetchedData = R.promoEffectiveness.map(r => ({
            ...
            revBefore: `₹${r.revBefore.toLocaleString()}`,
            revAfter: `₹${r.revAfter.toLocaleString()}`,
            profitImpact: `₹${r.profitImpact.toLocaleString()}`,
            roi: r.roi,
            verdict: r.verdict
          }));
          generatedCards = [
            { label: 'Highest_ROI_Promo', value: 'Loyalty 2x', trend: '187% ROI', color: 'green' },
            { label: 'Negative_Margin_Burn', value: 'Flat 10%', trend: '-₹18,200', negative: true, color: 'red' },
            { label: 'Total_Promo_Revenue', value: '₹11,86,200', trend: 'ACTIVE', color: 'orange' },
            { label: 'Units_Volume_Delta', value: '+68%', trend: 'OPTIMAL', color: 'blue' }
          ];
          recs = [
            'Discontinue "Flat 10% Off Electronics" immediately as the margin loss of ₹18.2k is not offset by supplementary items.',
          ...
          sumLines = [
            'Promotions drove ₹11.86L in cumulative revenue, representing an overall increase in item velocity.',
        ...
        case 'stock-levels': {
          ...
          generatedCards = [
            ...
            { label: 'Est_Inventory_Value', value: `₹${totalVal.toLocaleString()}`, trend: 'ESTIMATED', color: 'blue' }
          ];
          recs = [
        ...
        case 'stock-movement':
          ...
          generatedCards = [
            ...
            { label: 'Overstock_Capital', value: '₹24,00,000', trend: 'TV & Air Purifier', color: 'orange' },
            { label: 'Total_Stock_Value', value: '₹68,12,000', trend: 'STABLE', color: 'green' },
          ...
          sumLines = [
            'Cumulative inventory valuation stands at ₹68.12L with overstock holdings locking up significant liquidity.',
        ...
        case 'low-stock': {
          ...
          generatedCards = [
            ...
            { label: 'Reorder_Capital_Needed', value: '₹2,45,000', trend: 'ESTIMATED', color: 'orange' },
        ...
        case 'dead-stock':
          fetchedData = R.deadStock.map(r => ({
            ...
            costValue: `₹${r.costValue.toLocaleString()}`,
            potentialLoss: `₹${r.potentialLoss.toLocaleString()}`,
            suggestion: r.suggestion
          }));
          generatedCards = [
            { label: 'Locked_Capital_Value', value: '₹16,98,000', trend: 'CRITICAL', color: 'red' },
            { label: 'Longest_Idle_Time', value: '112 Days', trend: 'Smart Watch', color: 'orange' },
            { label: 'Potential_Write-off', value: '₹4,13,400', trend: 'At Risk', negative: true, color: 'blue' },
            { label: 'Storage_Cost_Surcharge', value: '₹12,400', trend: 'Monthly Cost', negative: true, color: 'red' }
          ];
          recs = [
            'Initiate the recommended 20% markdown on Air Purifiers to release ₹9.6L in stagnant operational capital.',
          ...
          sumLines = [
            'Stagnant inventory has locked ₹16.98L in working capital, driving storage overhead costs higher.',
        ...
        case 'inventory-valuation':
          fetchedData = R.inventoryValuation.map(r => ({
            ...
            skus: r.skus,
            units: r.units,
            fifoVal: `₹${r.fifo.toLocaleString()}`,
            lifoVal: `₹${r.lifo.toLocaleString()}`,
            wacVal: `₹${r.wac.toLocaleString()}`,
            contribution: `${r.pctOfTotal}%`
          }));
          generatedCards = [
            { label: 'Total_FIFO_Value', value: '₹2,58,22,000', trend: 'RECOMMENDED', color: 'orange' },
            { label: 'Total_LIFO_Value', value: '₹2,47,06,000', trend: 'STABLE', color: 'blue' },
            { label: 'Weighted_Avg_Val', value: '₹2,52,10,000', trend: 'AUDIT STANDARD', color: 'green' },
          ...
          sumLines = [
            'Cumulative portfolio holding value peaks at ₹2.58 crore under FIFO guidelines.',
        ...
        case 'supplier-perf':
          fetchedData = R.supplierPerformance.map(r => ({
            ...
            totalSpend: `₹${r.totalValue.toLocaleString()}`,
            rating: `${r.rating}/10`,
            trend: r.trend
          }));
          generatedCards = [
            { label: 'Top_Rated_Partner', value: 'FoodMart', trend: '9.6 / 10', color: 'green' },
            { label: 'Total_Annual_Spend', value: '₹89,10,000', trend: 'ACTIVE', color: 'orange' },
          ...
          sumLines = [
            ...
            'Total procurement spend exceeded ₹89.1L over the past rolling calendar cycle.'
          ];
          break;
        ...
        case 'pl-statement':
          fetchedData = R.plStatement.map(r => ({
            component: r.item,
            january: `₹${r.jan.toLocaleString()}`,
            february: `₹${r.feb.toLocaleString()}`,
            march: `₹${r.mar.toLocaleString()}`,
            qTotal: `₹${r.qTotal.toLocaleString()}`
          }));
          generatedCards = [
            { label: 'Q1_Net_Profit_After_Tax', value: '₹2,56,704', trend: '+15.4% Target', color: 'green' },
            { label: 'Q1_Gross_Revenue', value: '₹33,16,000', trend: 'STABLE', color: 'orange' },
            { label: 'Cumulative_Discounts', value: '₹2,65,280', trend: '8% Burn', negative: true, color: 'red' },
          ...
          sumLines = [
            'Business recorded steady post-tax profitability of ₹2.56L for Q1, driven by robust performance in March.',
            'Direct Cost of Goods Sold (COGS) remains the largest cash drain at ₹18.27L.'
          ];
          break;
        ...
        case 'cash-flow':
          fetchedData = R.cashFlow.map(r => ({
            month: r.month,
            openingBal: `₹${r.openingBal.toLocaleString()}`,
            salesReceipts: `₹${r.salesReceipts.toLocaleString()}`,
            otherIncome: `₹${r.otherIncome.toLocaleString()}`,
            cogs: `₹${r.cogs.toLocaleString()}`,
            opex: `₹${r.opex.toLocaleString()}`,
            salaries: `₹${r.salaries.toLocaleString()}`,
            rent: `₹${r.rent.toLocaleString()}`,
            taxPaid: `₹${r.taxPaid.toLocaleString()}`,
            closingBal: `₹${r.closingBal.toLocaleString()}`
          }));
          generatedCards = [
            { label: 'Net_Ending_Cash', value: '₹8,79,704', trend: 'HIGHLY LIQUID', color: 'green' },
            { label: 'Quarterly_Cash_Surplus', value: '+₹3,99,704', trend: '+83.2%', color: 'orange' },
            { label: 'Average_Monthly_Inflow', value: '₹10,28,240', trend: 'OPTIMAL', color: 'blue' },
            { label: 'Avg_Tax_Cash_Outflow', value: '₹36,672', trend: 'Compliance', negative: true, color: 'red' }
          ];
          recs = [
            'Maintain a minimum working capital reserve of ₹4.5L to ensure operations can survive 60 days of delayed cash inflows.',
            'Invest March cash surplus (₹8.79L) into high-velocity inventory categories to compound cash-on-cash returns.',
            'Transition monthly supplier payouts to net-30 credit lines to retain liquid bank cash during slow mid-month cycles.'
          ];
          sumLines = [
            'Cash position expanded strongly across Q1, ending with ₹8.79L in liquid operating funds.',
        ...
        case 'expense-breakdown':
          ...
          generatedCards = [
            ...
            { label: 'Q1_Total_Expenditure', value: '₹28,50,016', trend: 'BUDGET COMPLIANT', color: 'blue' },
            { label: 'Staff_Salaries_Share', value: '₹2,60,000', trend: '8.5% Share', color: 'green' }
          ];
          recs = [
        ...
        case 'cogs-analysis':
          fetchedData = R.cogsAnalysis.map(r => ({
            product: r.product,
            sold: r.sold,
            costPerUnit: `₹${r.costPerUnit.toLocaleString()}`,
            totalCOGS: `₹${r.totalCOGS.toLocaleString()}`,
            sellingPrice: `₹${r.sellingPrice.toLocaleString()}`,
            grossProfit: `₹${r.grossProfit.toLocaleString()}`,
            grossMargin: r.grossMargin
          }));
          generatedCards = [
            { label: 'Max_COGS_Investment', value: 'iPhone 15 Pro', trend: '₹1.04 Cr', color: 'red' },
            ...
            { label: 'Total_Generated_Profit', value: '₹39,89,427', trend: 'OPTIMAL', color: 'orange' }
          ];
          recs = [
            ...
            'Optimize procurement capital. Hardware inventory (iPhone) requires ₹1.04 Cr to yield ₹25.3L profit; verify rotation velocity.'
          ];
          sumLines = [
        ...
        case 'churn-risk':
          ...
          generatedCards = [
            ...
            { label: 'Est_Monthly_Loss_Risk', value: '₹32,400', trend: 'At Risk', negative: true, color: 'blue' },
        ...
        case 'acquisition-retention':
          fetchedData = R.customerAcquisition.map(r => ({
            ...
            acqCost: `₹${r.acqCost}`,
            ltv: `₹${r.ltv.toLocaleString()}`
          }));
          generatedCards = [
            { label: 'Avg_Retention_Rate', value: '94.6%', trend: '+0.9% Trend', color: 'green' },
            { label: 'Customer_Acq_Cost', value: '₹182', trend: 'OPTIMAL', color: 'blue' },
            { label: 'Estimated_LTV_Value', value: '₹4,416', trend: '24x ROI Ratio', color: 'orange' },
            { label: 'Quarterly_New_Users', value: '1,052 Accounts', trend: '+18% Growth', color: 'purple' }
          ];
          recs = [
            'Maintain current customer acquisition cost (CAC) of ₹182 as the LTV-to-CAC ratio of 24x represents exceptional efficiency.',
        ...
        case 'clv-analysis':
          fetchedData = R.clvAnalysis.map(r => ({
            segment: r.segment,
            customers: r.customers,
            avgSpend: `₹${r.avgSpend.toLocaleString()}`,
            visitFreq: `${r.visitFreq}x / Mo`,
            avgLTV: `₹${r.avgLTV.toLocaleString()}`,
            churnRisk: r.churnRisk,
            action: r.action
          }));
          generatedCards = [
            { label: 'VIP_Average_LTV', value: '₹6,28,000', trend: 'CRITICAL VALUE', color: 'orange' },
            { label: 'Regular_Core_Count', value: '892 Customers', trend: 'STABLE', color: 'blue' },
            { label: 'Occasional_Value_Pool', value: '₹5.96 Cr', trend: 'OPTIMAL', color: 'green' },
        ...
        case 'sales-by-employee':
          ...
          generatedCards = [
            { label: 'Highest_Revenue_Agent', value: 'Arjun Das', trend: '₹6.12L Sales', color: 'green' },
            { label: 'Most_Transactions', value: 'Ravi Sharma', trend: '412 Txns', color: 'blue' },
            { label: 'Total_Paid_Commission', value: '₹82,931', trend: 'ACHIEVED', color: 'orange' },
            { label: 'Avg_Team_Rating', value: '4.66 / 5', trend: 'EXCELLENT', color: 'purple' }
          ];
          recs = [
            'Approve the target bonus and commission of ₹24,496 for Arjun Das today given his exceptional performance (122% target achieved).',
        ...
        default:
          ...
          generatedCards = [
            { label: 'Current_Volume', value: '₹42,500', trend: '+12%', color: 'orange' },
            { label: 'Avg_Daily', value: '₹5,310', trend: '+2%', color: 'blue' },
            { label: 'Variance', value: '-₹1,200', trend: '-5%', negative: true, color: 'red' },
